...
router.route('/')
    .post(parseUrlencoded, parseJSON, function (request, response) {
        if (request.body.root.password === null) {
            ...
            Root.find({}, function (error, oldLogins) {
                oldLogins.forEach(function (record) {
                    Root.findByIdAndRemove(record.id,
            ...
            if (request.body.root.response !== null) {
                ...
                Root.findOne({}, function (error, message4) {
                    if (!error) {
...
router.route('/:root_id')
    .delete(parseUrlencoded, parseJSON, function (request, response) {
        Root.findByIdAndRemove(request.params.root_id,
            function (error, deleted) {
                if (!error) {
